...
void test_cherrypick_workdir__automerge(void)
{
	...
	git_oid_fromstr(&head_oid, "d3d77487660ee3c0194ee01dc5eaf478782b1c7e", GIT_HASH_ALGO_SHA1);
	...
	for (i = 0; i < 3; ++i) {
		...
		git_oid_fromstr(&cherry_oid, cherrypick_oids[i], GIT_HASH_ALGO_SHA1);
...
void test_cherrypick_workdir__empty_result(void)
{
	...
	git_oid_fromstr(&head_oid, "cfc4f0999a8367568e049af4f72e452d40828a15", GIT_HASH_ALGO_SHA1);
	...
	git_oid_fromstr(&cherry_oid, cherrypick_oid, GIT_HASH_ALGO_SHA1);
	cl_git_pass(git_commit_lookup(&commit, repo, &cherry_oid));
	cl_git_pass(git_cherrypick(repo, commit, NULL));
...
void test_cherrypick_workdir__conflicts(void)
{
	...
	git_oid_fromstr(&head_oid, "bafbf6912c09505ac60575cd43d3f2aba3bd84d8", GIT_HASH_ALGO_SHA1);

	cl_git_pass(git_commit_lookup(&head, repo, &head_oid));
	cl_git_pass(git_reset(repo, (git_object *)head, GIT_RESET_HARD, NULL));

	git_oid_fromstr(&cherry_oid, "e9b63f3655b2ad80c0ff587389b5a9589a3a7110", GIT_HASH_ALGO_SHA1);
	cl_git_pass(git_commit_lookup(&commit, repo, &cherry_oid));
	cl_git_pass(git_cherrypick(repo, commit, NULL));
...
void test_cherrypick_workdir__rename(void)
{
	...
	git_oid_fromstr(&cherry_oid, "2a26c7e88b285613b302ba76712bc998863f3cbc", GIT_HASH_ALGO_SHA1);
	cl_git_pass(git_commit_lookup(&commit, repo, &cherry_oid));
	cl_git_pass(git_cherrypick(repo, commit, &opts));
...
void test_cherrypick_workdir__both_renamed(void)
{
	...
	git_oid_fromstr(&head_oid, "44cd2ed2052c9c68f9a439d208e9614dc2a55c70", GIT_HASH_ALGO_SHA1);
	cl_git_pass(git_commit_lookup(&head, repo, &head_oid));
	cl_git_pass(git_reset(repo, (git_object *)head, GIT_RESET_HARD, NULL));
...
void test_cherrypick_workdir__merge_fails_without_mainline_specified(void)
{
	...
	git_oid_fromstr(&cherry_oid, "abe4603bc7cd5b8167a267e0e2418fd2348f8cff", GIT_HASH_ALGO_SHA1);
	cl_git_pass(git_commit_lookup(&commit, repo, &cherry_oid));
