...
void test_commit_write__from_memory(void)
{
   ...
   git_oid_fromstr(&tree_id, tree_id_str, GIT_HASH_ALGO_SHA1);
   cl_git_pass(git_tree_lookup(&tree, g_repo, &tree_id));

   git_oid_fromstr(&parent_id, parent_id_str, GIT_HASH_ALGO_SHA1);
...
void test_commit_write__can_write_invalid_objects(void)
{
	...
	git_oid_fromstr(&expected_id, "c8571bbec3a72c4bcad31648902e5a453f1adece", GIT_HASH_ALGO_SHA1);
	cl_git_pass(create_commit_from_ids(&commit_id, &tree_id, &parent_id));
	cl_assert_equal_oid(&expected_id, &commit_id);

	/* this is a wholly invented tree id */
	git_oid_fromstr(&tree_id, "1234567890123456789012345678901234567890", GIT_HASH_ALGO_SHA1);
	git_oid_fromstr(&parent_id, parent_id_str, GIT_HASH_ALGO_SHA1);

	git_oid_fromstr(&expected_id, "996008340b8e68d69bf3c28d7c57fb7ec3c8e202", GIT_HASH_ALGO_SHA1);
	cl_git_pass(create_commit_from_ids(&commit_id, &tree_id, &parent_id));
	cl_assert_equal_oid(&expected_id, &commit_id);

	/* this is a wholly invented parent id */
	git_oid_fromstr(&tree_id, tree_id_str, GIT_HASH_ALGO_SHA1);
	git_oid_fromstr(&parent_id, "1234567890123456789012345678901234567890", GIT_HASH_ALGO_SHA1);

	git_oid_fromstr(&expected_id, "d78f660cab89d9791ca6714b57978bf2a7e709fd", GIT_HASH_ALGO_SHA1);
	cl_git_pass(create_commit_from_ids(&commit_id, &tree_id, &parent_id));
	cl_assert_equal_oid(&expected_id, &commit_id);

	/* these are legitimate objects, but of the wrong type */
	git_oid_fromstr(&tree_id, parent_id_str, GIT_HASH_ALGO_SHA1);
	git_oid_fromstr(&parent_id, tree_id_str, GIT_HASH_ALGO_SHA1);

	git_oid_fromstr(&expected_id, "5d80c07414e3f18792949699dfcacadf7748f361", GIT_HASH_ALGO_SHA1);
	cl_git_pass(create_commit_from_ids(&commit_id, &tree_id, &parent_id));
	cl_assert_equal_oid(&expected_id, &commit_id);
}
