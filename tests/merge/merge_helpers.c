...
static int index_entry_eq_merge_index_entry(const struct merge_index_entry *expected, const git_index_entry *actual)
{
	...
	if (strlen(expected->oid_str) != 0) {
		cl_git_pass(git_oid_fromstr(&expected_oid, expected->oid_str, GIT_HASH_ALGO_SHA1));
		test_oid = 1;
	} else
		test_oid = 0;
	...
	*/
	...
	for (i = 0; i < expected_len; i++) {
		...
		if (expected[i].ancestor_mode > 0) {
			cl_git_pass(git_oid_fromstr(&expected_oid, expected[i].ancestor_oid_str, GIT_HASH_ALGO_SHA1));
		...
		if (expected[i].our_mode > 0) {
			cl_git_pass(git_oid_fromstr(&expected_oid, expected[i].our_oid_str, GIT_HASH_ALGO_SHA1));
		...
		if (expected[i].their_mode > 0) {
			cl_git_pass(git_oid_fromstr(&expected_oid, expected[i].their_oid_str, GIT_HASH_ALGO_SHA1));
...
int merge_test_workdir(git_repository *repo, const struct merge_index_entry expected[], size_t expected_len)
{
	...
	for (i = 0; i < expected_len; i++) {
		...
		git_oid_fromstr(&expected_oid, expected[i].oid_str, GIT_HASH_ALGO_SHA1);
