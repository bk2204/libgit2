	...
	const char **expected_oids)
{
	...
	while (!(error = git_iterator_advance(&entry, i))) {
		...
		if (expected_oids != NULL) {
			git_oid oid;
			cl_git_pass(git_oid_fromstr(&oid, expected_oids[count], GIT_HASH_ALGO_SHA1));
...
static void create_paths(git_index *index, const char *root, int depth)
{
	...
	for (i = 0; i < 8; i++) {
		...
		if (file) {
			...
			git_oid_fromstr(&entry.id, "d44e18fb93b7107b5cd1b95d601591d77869a1b6", GIT_HASH_ALGO_SHA1);
	...
	const char *their_path)
{
	...
	git_oid_fromstr(&ancestor.id, "d44e18fb93b7107b5cd1b95d601591d77869a1b6", GIT_HASH_ALGO_SHA1);
	GIT_INDEX_ENTRY_STAGE_SET(&ancestor, 1);

	ours.path = our_path;
	ours.mode = GIT_FILEMODE_BLOB;
	git_oid_fromstr(&ours.id, "d44e18fb93b7107b5cd1b95d601591d77869a1b6", GIT_HASH_ALGO_SHA1);
	GIT_INDEX_ENTRY_STAGE_SET(&ours, 2);

	theirs.path = their_path;
	theirs.mode = GIT_FILEMODE_BLOB;
	git_oid_fromstr(&theirs.id, "d44e18fb93b7107b5cd1b95d601591d77869a1b6", GIT_HASH_ALGO_SHA1);
