...
static void test_read_object(object_data *data)
{
    ...
    cl_git_pass(git_oid_fromstr(&id, data->id, GIT_HASH_ALGO_SHA1));
...
void test_odb_loose__exists(void)
{
	...
	cl_git_pass(git_oid_fromstr(&id, one.id, GIT_HASH_ALGO_SHA1));
	cl_assert(git_odb_exists(odb, &id));

	cl_git_pass(git_oid_fromstrp(&id, "8b137891", GIT_HASH_ALGO_SHA1));
	cl_git_pass(git_odb_exists_prefix(&id2, odb, &id, 8));
	cl_assert_equal_i(0, git_oid_streq(&id2, one.id));

	/* Test for a missing object */
	cl_git_pass(git_oid_fromstr(&id, "8b137891791fe96927ad78e64b0aad7bded08baa", GIT_HASH_ALGO_SHA1));
	cl_assert(!git_odb_exists(odb, &id));

	cl_git_pass(git_oid_fromstrp(&id, "8b13789a", GIT_HASH_ALGO_SHA1));
