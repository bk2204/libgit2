...
void test_index_cache__write_extension_at_root(void)
{
	...
	cl_git_pass(git_oid_fromstr(&id, tree_id_str, GIT_HASH_ALGO_SHA1));
...
void test_index_cache__read_tree_no_children(void)
{
	...
	cl_assert(index->tree == NULL);
	cl_git_pass(git_oid_fromstr(&id, "45dd856fdd4d89b884c340ba0e047752d9b085d6", GIT_HASH_ALGO_SHA1));
	cl_git_pass(git_tree_lookup(&tree, g_repo, &id));
	cl_git_pass(git_index_read_tree(index, tree));
	git_tree_free(tree);
	...
	entry.path = "new.txt";
	entry.mode = GIT_FILEMODE_BLOB;
	git_oid_fromstr(&entry.id, "d4bcc68acd4410bf836a39f20afb2c2ece09584e", GIT_HASH_ALGO_SHA1);
...
void test_index_cache__two_levels(void)
{
	...
	entry.mode = GIT_FILEMODE_BLOB;
	cl_git_pass(git_oid_fromstr(&entry.id, "a8233120f6ad708f843d861ce2b7228ec4e3dec6", GIT_HASH_ALGO_SHA1));
	...
	entry.path = "top-level.txt";
	cl_git_pass(git_oid_fromstr(&entry.id, "3697d64be941a53d4ae8f6a271e4e3fa56b022cc", GIT_HASH_ALGO_SHA1));
	cl_git_pass(git_index_add(index, &entry));
