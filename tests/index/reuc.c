...
void test_index_reuc__add(void)
{
	...
	git_oid_fromstr(&ancestor_oid, ONE_ANCESTOR_OID, GIT_HASH_ALGO_SHA1);
	git_oid_fromstr(&our_oid, ONE_OUR_OID, GIT_HASH_ALGO_SHA1);
	git_oid_fromstr(&their_oid, ONE_THEIR_OID, GIT_HASH_ALGO_SHA1);
...
void test_index_reuc__read_bypath(void)
{
	...
	cl_assert(reuc->mode[1] == 0100644);
	cl_assert(reuc->mode[2] == 0100644);
	git_oid_fromstr(&oid, TWO_ANCESTOR_OID, GIT_HASH_ALGO_SHA1);
	cl_assert_equal_oid(&reuc->oid[0], &oid);
	git_oid_fromstr(&oid, TWO_OUR_OID, GIT_HASH_ALGO_SHA1);
	cl_assert_equal_oid(&reuc->oid[1], &oid);
	git_oid_fromstr(&oid, TWO_THEIR_OID, GIT_HASH_ALGO_SHA1);
	...
	cl_assert(reuc->mode[1] == 0100644);
	cl_assert(reuc->mode[2] == 0100644);
	git_oid_fromstr(&oid, ONE_ANCESTOR_OID, GIT_HASH_ALGO_SHA1);
	cl_assert_equal_oid(&reuc->oid[0], &oid);
	git_oid_fromstr(&oid, ONE_OUR_OID, GIT_HASH_ALGO_SHA1);
	cl_assert_equal_oid(&reuc->oid[1], &oid);
	git_oid_fromstr(&oid, ONE_THEIR_OID, GIT_HASH_ALGO_SHA1);
	cl_assert_equal_oid(&reuc->oid[2], &oid);
}
...
void test_index_reuc__updates_existing(void)
{
	...
	git_oid_fromstr(&ancestor_oid, TWO_ANCESTOR_OID, GIT_HASH_ALGO_SHA1);
	git_oid_fromstr(&our_oid, TWO_OUR_OID, GIT_HASH_ALGO_SHA1);
	git_oid_fromstr(&their_oid, TWO_THEIR_OID, GIT_HASH_ALGO_SHA1);
