	...
	git_filemode_t expected_filemode)
{
	...
	cl_git_pass(git_oid_fromstr(&oid, expected_sha, GIT_HASH_ALGO_SHA1));
...
static void two_blobs(git_treebuilder *bld)
{
	...
	cl_git_pass(git_oid_fromstr(&oid,
		...
		GIT_HASH_ALGO_SHA1));	/* blob oid (README) */
	...
	cl_git_pass(git_oid_fromstr(&oid,
		...
		GIT_HASH_ALGO_SHA1));	/* blob oid (new.txt) */
...
static void add_fake_conflicts(git_index *index)
{
	...
	git_oid_fromstr(&ancestor_entry.id, "a8233120f6ad708f843d861ce2b7228ec4e3dec6", GIT_HASH_ALGO_SHA1);

	our_entry.path = "duplicate";
	our_entry.mode = GIT_FILEMODE_BLOB;
	GIT_INDEX_ENTRY_STAGE_SET(&our_entry, 2);
	git_oid_fromstr(&our_entry.id, "45b983be36b73c0788dc9cbcb76cbb80fc7bb057", GIT_HASH_ALGO_SHA1);

	their_entry.path = "duplicate";
	their_entry.mode = GIT_FILEMODE_BLOB;
	GIT_INDEX_ENTRY_STAGE_SET(&their_entry, 3);
	git_oid_fromstr(&their_entry.id, "a71586c1dfe8a71c6cbf6c129f404c5642ff31bd", GIT_HASH_ALGO_SHA1);
