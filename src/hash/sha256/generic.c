...
static void transform(git_hash_sha256_ctx *ctx, const unsigned char *buf)
{
...
int git_hash_sha256_final(git_oid *out, git_hash_sha256_ctx *ctx)
{
	...
	i = ctx->size & 63;
	git_hash_sha256_update(ctx, pad, 1 + (63 & (55 - i)));
	git_hash_sha256_update(ctx, padlen, 8);

	/* copy output */
	for (i = 0; i < 8; i++)
		put_be32(out->id + i*sizeof(uint32_t), ctx->state[i]);
